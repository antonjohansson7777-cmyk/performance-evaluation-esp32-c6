...
static void throughput_task(void *pvParameters)
{
    ...
    if (sock < 0) { ESP_LOGE(TAG, "throughput socket fel"); vTaskDelete(NULL); return; }
    ...
    ESP_LOGI(TAG, "Skickar throughput till: %s port %d", target_str, THROUGHPUT_PORT);
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Ansluten! Startar throughput + load-server om 2s...");
