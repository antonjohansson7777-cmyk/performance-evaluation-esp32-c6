/*
 * Zigbee HA_on_off_switch — Latency test med load
 *
 * Två parallella loopar:
 * Ping-loop: Toggle + Read Attribute, mäter RTT, 100ms interval
 * Load-loop: Write Attribute (on_time), genererar bakgrundstrafik
 *
 * Justera LOAD_INTERVAL_MS för att styra lastintensitet:
 * 100 ms → ~10 paket/s (lätt last)
...
/* ===== Konfigurerbart ===== */
#define PING_INTERVAL_MS 1000 /* interval mellan pings */
#define LOAD_INTERVAL_MS 50   /* interval för load-paket — justera detta */
...
static void send_ping(void) {
    ...
    esp_zb_lock_release();
    /* Read Attribute — svaret är vår "pong" */
    send_read_attr();
}
...
static void zb_buttons_handler(switch_func_pair_t *button_func_pair) {
    if (button_func_pair->func == SWITCH_ONOFF_TOGGLE_CONTROL) {
        ...
        ESP_LOGI(TAG, "Startar latenstest med load (%d ms load-interval)", LOAD_INTERVAL_MS);
        stats_init();
